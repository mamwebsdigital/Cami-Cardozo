...
function showImage(index) {
  ...
  temp.onload = () => { modalImg.src = temp.src; };
...
function tagOrientation(imgEl){
  if (imgEl.naturalHeight > imgEl.naturalWidth) {
    imgEl.classList.add("is-portrait");
  } else {
...
products.forEach(product => {
  ...
  img.loading = "lazy";
  img.decoding = "async";
  img.addEventListener("load", () => tagOrientation(img));
  ...
  // Modal al click en imagen principal
  ...
  product.images.forEach((thumbSrc, index) => {
    ...
    thumb.addEventListener("load", () => tagOrientation(thumb));

    thumb.addEventListener("click", () => {
      img.src = thumbSrc;
      img.onload = () => tagOrientation(img);
      currentIndex = index;
...
function showCartNotif() {
  ...
  if (keepShopping) {
    keepShopping.onclick = () => notifBox.classList.remove("show");
  }
